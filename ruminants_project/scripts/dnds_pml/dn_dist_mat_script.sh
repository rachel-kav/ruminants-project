...
OUTPUT_DIR="/Users/qjs599/Desktop/ruminants/ruminants_project/data/dn_trees"

mkdir -p "$OUTPUT_DIR"

# Iterate over subdirectories
for SUBDIR in "$BASE_DIR"/*/; do
  # Remove trailing slash and extract folderqname
  FOLDER_NAME=$(basename "$SUBDIR")
  
  # Define input and output file paths
  INPUT_FILE="$SUBDIR/2ML.dN"
  ...
  if [ -f "$INPUT_FILE" ]; then
    ...
    echo "Skipping $FOLDER_NAME: 2ML.dN not found."
  fi
done
